...
import edu.seu.couponhub.engine.common.enums.CouponTemplateStatusEnum;
import edu.seu.couponhub.engine.dao.entity.CouponTemplateDO;
import edu.seu.couponhub.engine.dao.mapper.CouponTemplateMapper;
import edu.seu.couponhub.engine.dao.sharding.DBShardingUtil;
...
public class CouponTemplateServiceImpl extends ServiceImpl<CouponTemplateMapper, CouponTemplateDO> implements CouponTemplateService {
    ...
    public CouponTemplateQueryRespDTO findCouponTemplate(CouponTemplateQueryReqDTO requestParam) {
        ...
        String couponTemplateCacheKey = String.format(EngineRedisConstant.COUPON_TEMPLATE_KEY,
        ...
        if (MapUtil.isEmpty(couponTemplateCacheMap)) {
            ...
            String couponTemplateIsNullCacheKey = String.format(EngineRedisConstant.COUPON_TEMPLATE_IS_NULL_KEY,
                    requestParam.getCouponTemplateId());
            ...
            // 获取优惠券模板分布式锁
            RLock lock = redissonClient.getLock(String.format(EngineRedisConstant.LOCK_COUPON_TEMPLATE_KEY,
                    requestParam.getCouponTemplateId()));
            lock.lock();
            ...
            try {
                ...
                if (MapUtil.isEmpty(couponTemplateCacheMap)) {
                    Result<edu.seu.couponhub.api.dto.resp.CouponTemplateQueryRespDTO> remoteResult =
                            merchantAdminRemoteService.findCouponTemplate(
                            requestParam.getShopNumber(),
                            requestParam.getCouponTemplateId()
                    );

                    // 优惠券模板不存在或者已过期加入空值缓存，并且抛出异常
                    if (remoteResult == null || remoteResult.isFail() || remoteResult.getData() == null) {
                    ...
                    CouponTemplateQueryRespDTO actualRespDTO = BeanUtil.toBean(remoteResult.getData(),
                    ...
                    args.add(String.valueOf(actualRespDTO.getValidEndTime().getTime() / 1000));
        ...
        return BeanUtil.mapToBean(couponTemplateCacheMap, CouponTemplateQueryRespDTO.class, false,
                CopyOptions.create());
    }
